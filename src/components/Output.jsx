...
const Output = ({ messageObj }) => {
    return (
        <div className="min-h-[156px] max-w-[357px] max-w-[357px] tablet:min-w-[357px] tablet:max-w-[459px] lg:min-w-[459px] lg:max-w-[509px]">
            <div
...
`}>
                {messageObj.messageValue === 0 && (
                    <>
                        <p>
                            Press the microphone. While it's blinking instruct the blue square to
                            move about the maze using the words "left", "right", "up" and "down".
                        </p>
                        <p> Cross the FINISH line to win!</p>
                    </>
                )}
                {messageObj.messageValue === 1 && <p>{messageObj.message}</p>}
                ...
                {messageObj.messageValue === 3 && <p>{messageObj.message}</p>}
            </div>
        </div>
    );
};
